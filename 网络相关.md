...
* > 接口地址：GET http://example.com/<page>/get_data.json  
> 在这个接口当中，\<page>部分代表页数，传入不同页数，服务器返回的数据也会不同  
 
&emsp;&emsp;在```@GET```注解指定的接口地址中，使用了一个```{page}```的占位符，在```getData()```方法中添加了一个```page```参数，并使用```@Path("page")```注解来声明这个参数。  
&emsp;&emsp;当调用```getData()```方法发起请求地址时，```Retrofit```就会自动将```page```参数的值替换到占位符的位置，从而组成一个合法的请求地址。  
```kotlin
